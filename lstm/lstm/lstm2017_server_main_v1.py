...
def evaluating(ipsource, ipdestination, sourcePort, dstPort, protocol,timeStamp, pred_label):
    global totalFlows, ip_attacker,ip_victim,currentAttack,file1, respuestas, buenos, malos
    ...
    if pred_label != "incomplete": # systems with memory
        ...
        with open(file1,"a") as f:
            ...
            if str(ipsource).find("5") > -1 or str(ipsource).find("6") > -1:
                    malos = malos + 1
            else:
                    buenos = buenos + 1
            ...
            resp = str(pred_label) + "-" + str(tstmp) + "-" + str(real_label) + "-" + str(buenos) + "-" + str(malos)
            respuestas[llave] = resp
            #print(respuestas)
    return 0
...
def respond():
        global repuestas
        respuestas2 = respuestas.copy()
        respuestas.clear()
        return {"data":respuestas2},202
